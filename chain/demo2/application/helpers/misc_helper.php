...
// returns true or false for multiple currencies usage
...
}if (!function_exists('pt_get_city_data')) {

		function pt_get_city_data($id) {
				$CI = get_instance();
				$CI->load->model('countries_model');
				$res = $CI->countries_model->get_city_data($id);
				return $res;
		}
...
}if (!function_exists('pt_get_city_name')) {

		function pt_get_city_name($city) {
				$CI = get_instance();
				$CI->load->model('admin/countries_model');
				$res = $CI->countries_model->get_city_name($city);
				return $res;
		}
		...
		function city_name($city) {
				$CI = get_instance();
				$CI->load->model('admin/countries_model');
				$res = $CI->countries_model->city_name($city);
				return $res;
		}
...
}if (!function_exists('pt_videos_count')) {

		function pt_videos_count($hotelid) {
				$CI = get_instance();
				$CI->load->model('hotels/hotels_model');
				$res = $CI->hotels_model->videos_count($hotelid);
				return $res;
		}
		...
		function pt_pendings_count($type, $module = null) {
				...
				if ($type = 'videos') {
						if ($module == 'hotels') {
								$counts['videos']['hotels'] = $CI->misc_model->count_pending_hotel_videos();
						}
						elseif ($module == "tours") {
								$counts['videos']['tours'] = $CI->misc_model->count_pending_tour_videos();
						}
						elseif ($module == "cruises") {
								$counts['videos']['cruises'] = $CI->misc_model->count_pending_cruise_videos();
						}
				}
...
}if (!function_exists('pt_weather_icons')) {

		function pt_weather_icons($status) {
				if ($status == 'sky is clear') {
						return '2';
				}
				elseif ($status == 'few clounds') {
						return '1';
				}
				elseif ($status == 'scattered clouds') {
						return '5';
				}
				elseif ($status == 'broken clouds') {
						return '1';
				}
				elseif ($status == 'shower rain') {
						return '9';
				}
				elseif ($status == 'Rain') {
						return '3';
				}
				elseif ($status == 'Thunderstorm') {
						return '8';
				}
				elseif ($status == 'snow') {
						return '7';
				...
				elseif ($status == 'heavy intensity rain') {
						return '9';
				}
		}
...
// Display the map.
		...
		function pendings_result() {
				...
				$hotelvidscount = pt_pendings_count('videos', 'hotels');
				$hvcounts = $hotelvidscount['videos']['hotels'];
				$total['img_rooms'] = $ricounts;
				$total['img_hotels'] = $hicounts;
				$total['vid_hotels'] = $hvcounts;
				...
				$cruisevidscount = pt_pendings_count('videos', 'cruises');
				$cvcounts = $cruisevidscount['videos']['cruises'];
				$total['img_crooms'] = $croomicounts;
				$total['img_cruises'] = $cricounts;
				$total['vid_cruises'] = $cvcounts;
				...
				$tourvidscount = pt_pendings_count('videos', 'tours');
				$ticounts = $tourimgscount['images']['tours'];
				$tvcounts = $tourvidscount['videos']['tours'];
				$total['img_tours'] = $ticounts;
				$total['vid_tours'] = $tvcounts;
				...
				$totalvids = $hvcounts + $tvcounts + $cvcounts;
				$total['totalreviews'] = $totalreviews;
				$total['totalimages'] = $totalimages;
				$total['totalvids'] = $totalvids;
				$total['grandtotal'] = $totalvids + $totalimages + $totalreviews + $custcount + $supplierscount;
				return $total;
		}
...
}if (!function_exists('sms_template_detail')) {

		function sms_template_detail($id) {
				$CI = get_instance();
				$CI->load->model('admin/templates_model');
				return $CI->templates_model->get_sms_template_detail($id);
		}

}if (!function_exists('sendsms')) {

		function sendsms($message, $receiver, $template) {
				$CI = get_instance();
				$CI->load->library('smsaddonlib');
				$CI->smsaddonlib->send_sms($message, $receiver, $template);
		}

}if (!function_exists('CityStateCountry')) {

		function CityStateCountry($txt) {
				$CI = get_instance();
				if (!empty ($txt)) {
						$CI->db->like('city_name', $txt);
						$rscity = $CI->db->get('pt_cities')->result();
						$CI->db->like('state_name', $txt);
						$rsstates = $CI->db->get('pt_states')->result();
						$CI->db->like('short_name', $txt);
						$rscountry = $CI->db->get('pt_countries')->result();
						if (!empty ($rscity)) {
								return "city";
						}
						elseif (!empty ($rsstates)) {
								return "state";
						}
						elseif (!empty ($rscountry)) {
								return "country";
						}
				}
				else {
