...
class Theme
{
	...
	public function _getCustomTheme($client_id){
		$this->_ci->db->select("meta_key,meta_value");
        $this->_ci->db->where('client_id',$client_id);
        $data = $this->_ci->db->get('client_meta')->result();
		$hasil = array();
		foreach ($data as $k => $v) {
			$hasil[$v->meta_key] = $v->meta_value;
		}
		return $hasil;
	}
	
	public function _getClientType($client_id=0){
		$result = new stdClass;
		$client	= (explode(".",$_SERVER['HTTP_HOST']));
		$client = $client[0];
		$this->_ci->db->select("client_type,client_id");
		if($client_id>0)
			$this->_ci->db->where('client_id',$client_id);
		else
        	$this->_ci->db->where('client_slug',$client);
        $r = $this->_ci->db->get('clients')->result();
        $result->type = $r[0]->client_type;
		$result->theme =$this->_getCustomTheme($r[0]->client_id);
		return $result;
	}
