...
// $city = api_get_city_name($r->hotel_city);
		...
		function get_featured_hotels() {
				...
				if (!empty ($rs)) {
						...
						foreach ($rs as $r) {
								...
								$result["response"][] = array('id' => $r->hotel_id, 'title' => $r->hotel_title, 'description' => limit_chars(strip_tags($r->hotel_desc), 75), 'price' => $price, 'thumbnail' => $this->hotel_thumbnail($r->hotel_id), 'city' => $city[0]->city_name, 'stars' => $r->hotel_stars);
...
//  $data['all']  = $this->db->get('pt_hotels',$perpage,$offset)->result();
  ...
  pt_hotels.hotel_stars,pt_hotels.hotel_basic_price,pt_hotels.hotel_status,pt_cities.*,pt_states.*,pt_countries.*');

//  $this->db->select_avg('pt_reviews.review_overall','overall');

				$this->db->join('pt_cities', 'pt_hotels.hotel_city = pt_cities.city_id', 'left');
