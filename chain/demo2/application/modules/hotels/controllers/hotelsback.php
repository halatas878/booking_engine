...
class hotelsback extends MX_Controller {
		...
		function index() {
				...
				$xcrud->where('hotel_id',$this->session->userdata('hotel'));
...
// Add Hotels
		...
		function listrates(){
			$this->data['supplierHotels'] = $this->hotels_model->all_hotels($this->data['userloggedin']);
			$this->data['allhotels'] = $this->hotels_model->all_hotels();
			
			if(isset($_GET['hotel'])){
				 $this->load->library('hotels/hotels_lib');
				($_GET['hotel']>0)?$hotelid=$_GET['hotel']:$hotelid=0;
				($_GET['room']>0)?$roomid=$_GET['room']:$roomid=0;
               
                $this->data['rooms'] = $this->rooms_model->getHotelonRooms($roomid,$hotelid);
                $this->data['roomid'] = $roomid;
				 #echo "<pre/>";print_r($this->data['room']);exit();
			}
			
			
			$this->data['main_content'] = 'hotels/rates/lists';
			$this->data['page_title'] = 'Rates & Allotments';
			$this->load->view('admin/template', $this->data);
		...
		function setuprates(){
			...
			if(isset($_GET['room'])){
				...
				($_GET['hotel']>0)?$hotelid=$_GET['hotel']:$hotelid=0;
				($_GET['room']>0)?$roomid=$_GET['room']:$roomid=0;
                $this->data['rooms'] = $this->hotels_model->getShortRoom($roomid,"room");
                $this->data['roomid'] = $roomid;
			}
			
			
			$this->data['main_content'] = 'hotels/rates/setup_rates';
			$this->data['page_title'] = 'Rates & Allotments';
			$this->load->view('admin/template', $this->data);
		...
		function updaterates(){
			if(isset($_GET['room'])){
				...
				($_GET['hotel']>0)?$hotelid=$_GET['hotel']:$hotelid=0;
				($_GET['room']>0)?$roomid=$_GET['room']:$roomid=0;
                $this->data['lists'] = $this->hotels_model->getListPrice($roomid);
				$this->data['room'] = $this->hotels_model->getShortRoom($roomid,"room");
                $this->data['roomid'] = $roomid;
			}
			if(isset($_GET['data'])){
				$post = $this->data['post']	=	$this->rooms_model->getPricesList($_GET['data'])[0];
			}
			if(isset($_GET['fromdate'])){
				$this->data['listrate'] = $this->hotels_model->getRangePrice(urldecode($_GET['fromdate']),urldecode($_GET['todate']));
			}
			$this->data['main_content'] = 'hotels/rates/update_rates';
			$this->data['page_title'] = 'Rates & Allotments';
			$this->load->view('admin/template', $this->data);
		}	
		
		
		function cancelations($slug=''){
			
			if(isset($_POST['title'])){
				$update	= $this->hotels_model->updateCancelations($_POST);
				redirect($this->uri->segment(1).'/hotels/cancelations');
			}
			(isset($_GET['data']))?$id=$_GET['data']:$id=0;
			$this->data['lists'] = $this->hotels_model->getCancelations($id);
			
			$this->data['main_content'] = 'hotels/cancel/home';
			$this->data['page_title'] = 'Rates & Allotments';
			$this->load->view('admin/template', $this->data);
		}
		
		function youtube($slug=''){
			$data = $this->data['lists'] = $this->hotels_model->youtube();
			
			if(!empty($_POST['title'])){
				$_POST['user']	= $this->session->userdata('pt_logged_id');
				$_POST['update_date'] = date("Y-m-d H:i:s");
				$data = $this->hotels_model->youtubeData($_POST);
				#print_r($_POST);exit();
				redirect($this->uri->segment(1).'/hotels/youtube');
			}
			$this->data['main_content'] = 'hotels/youtube';
		...
		function contact($slug=''){
			if(isset($_POST['hotelphone'])){
				$update = $this->hotels_model->updateContact($_POST);
				redirect($this->uri->segment(1).'/hotels/contact');
			}
			$this->data['contact']	= $this->hotels_model->getContact($this->session->userdata('hotel'));
			$this->data['main_content'] = 'hotels/contact';
		...
		function analytics($slug=''){
			...
			$this->data['main_content'] = 'hotels/analytics';
...
if(!$this->data['addpermission'] && !$this->editpermission && !$this->deletepermission){
				...
				if ($args == 'add') {
						...
						if (!empty ($addroom)) {
								...
								$this->data['hotels'] = $this->hotels_model->all_hotels_names($userid,"session");
		...
		$output .= '<input type="hidden" name="ids_list" value="'.$ids_list.'">';
					   ...
					   //	 $datefmt = $this->input->post('dateformat');
						...
						#$xcrud->columns('room_title,pt_hotels.hotel_title,pt_hotels_types_settings.sett_name,room_quantity,room_basic_price,extra_bed,extra_bed_charges,room_min_stay,room_status');
						$xcrud->columns('room_title,pt_hotels.hotel_title,pt_hotels_types_settings.sett_name,room_quantity,room_basic_price,extra_bed,room_min_stay,room_status');
                        ...
                        $xcrud->where('pt_hotels.hotel_owned_by',$this->data['userloggedin']);
						 ...
						 $xcrud->where('pt_hotels.hotel_id',$this->session->userdata('hotel'));
						                        $xcrud->label('room_title','Name')->label('pt_hotels.hotel_titl','Hotel')->label('room_basic_price','Rates')->label('room_quantity','Allotments')->label('room_status','Status');
						$xcrud->label('pt_hotels_types_settings.sett_name', 'Room Type');
						$xcrud->label('room_min_stay', 'Gallery');
						$xcrud->label('extra_bed', 'Rates & Allotments');
...
//	$xcrud->unset_add();
		...
		function promotions($slug){
			if(isset($_POST['start_booking_period'])){
				$_POST['type'] = $slug;
				$post	= $this->hotels_model->setPromotions($_POST);
				redirect($this->uri->segment(1).'/hotels/promotions/'.$slug.'?room='.$_GET['room']);
			}
			if(isset($_GET['room'])){
				$rom = $this->data['room'] = $this->rooms_model->getHotelonRooms($_GET['room'],'');
			}
			if(isset($_GET['data'])){
				$id = $_GET['data'];
				$promote = $this->data['promote'] = $this->hotels_model->getPromote($id);
			}
			if(isset($_GET['room']) && !isset($_GET['temp'])){
				$promot = $this->data['promotions'] = $this->hotels_model->getPromotionsbyRoom($_GET['room'],$slug);
			}

			/*conditional property*/
			($promote->multiple_apply=="1")?$extraM='checked':$extraM='';
			...
			switch ($slug) {
				case 'hot-deals':
					$tr .='
						<tr>
							<td><span class="labe">Book within</span></td>
							<td><input type="text" name="book_within" value="'.$promote->book_within.'"  class="form-control input-sm"  style="width: 100px; display: inline-block;"/>
			        			<span class="labe">day(s)</span>
			        		</td>
			        		<td><span class="labe">Minimum</span></td>
		    				<td><input type="text" name="minimum_day"  value="'.$promote->minimum_days.'" class="form-control input-sm"  style="width: 100px; display: inline-block;"/>
			        			<span class="labe">days in advance</span>
			        		</td>
		    			 	<td></td>
						</tr>
					';
				...
				case 'early-bird':
					$tr .= '
						<tr>
							<td><span class="labe">Minimum</span></td>
		    				<td><input type="text" name="minimum_day"  value="'.$promote->minimum_days.'" class="form-control input-sm"  style="width: 100px; display: inline-block;"/>
			        		<span class="labe">days in advance</span></td>
			        		<td><!--input type="checkbox" name="multiple_apply" '.$extraM.'   value="1" class="form-control input-sm"  style="width: 100px; display: inline-block;"/>
			        			<span class="labe">Multiple Apply</span--></td>
			        		<td></td>
			        		<td></td>
						</tr>
					';
				...
				case 'minimum-stay':
					$tr .= '
						<tr>
							<td>Minimum Stay</td>
		    				<td><input type="text" name="minimum_stay" value="'.$promote->minimum_stay.'"  class="form-control input-sm"  style="width: 100px; display: inline-block;"/>
			        			<span class="labe">Night(s)</span></td>
			        		<td></td>
			        		<td></td>
			        		<td></td>
						</tr>
					...
					case 'bonus-night':
						$tr .= '
							<tr>
							<td><span class="labe">Pay</span></td>
							<td><input type="text" name="pay" value="'.$promote->stay.'"  class="form-control input-sm"  style="width: 100px; display: inline-block;"/>
			        			<span class="labe">Night(s)</span>
			        		</td>
			        		<td><span class="labe">Stay</span></td>
		    				<td><input type="text" name="stay"  value="'.$promote->pay.'" class="form-control input-sm"  style="width: 100px; display: inline-block;"/>
			        			<span class="labe">Night(s)</span>
			        		</td>
		    			 	<td></td>
						</tr>	
				...
				$this->data['slug'] = $slug;
				$this->data['tr'] = $tr;
                $this->data['main_content'] = 'hotels/promotions/index';
				$this->data['page_title'] = ucwords(str_replace("-", " ", $slug));
				$this->load->view('admin/template', $this->data);
		}
