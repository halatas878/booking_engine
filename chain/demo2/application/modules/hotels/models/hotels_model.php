...
// Get hotel settings data for adding hotel or room
	 ...
	 function updateSettingsTypeTranslation($postdata,$id) {
       ...
       foreach($postdata as $lang => $val){
		...
		function getRangePrice($from,$to){
			$from =date("Y-m-d", strtotime(str_replace("/","-",$from)));
			$to = date("Y-m-d", strtotime(str_replace("/","-",$to)));
		
			$this->db->select("ratetiers_date,rate_bar,	allotments,role_id,in_active,cancel_title,cancel_desc");
			$this->db->where("(DATE_FORMAT(ratetiers_date,'%Y-%m-%d') >= DATE_FORMAT('$from','%Y-%m-%d')) && (DATE_FORMAT(ratetiers_date,'%Y-%m-%d') <= DATE_FORMAT('$to','%Y-%m-%d')) ");
			$result = $this->db->get('fullratetiers')->result();
			$data = array();
			foreach($result as $k=>$v):
				$data[$v->ratetiers_date]=$v;
			endforeach;
			return $data;
		}
