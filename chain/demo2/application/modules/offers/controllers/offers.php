...
class Offers extends MX_Controller {
		...
		public function index() {
				...
				$check = $this->special_offers_model->offerExists($offername);
  				if ($check && !empty($offername)) {
  					...
  					$this->data['offer'] = $this->offers_lib->offer_details();
                    
                      $this->data['page_title'] =  $this->data['offer']->title;
                      if(!empty($sendmsg)){
                      	$this->load->model('admin/emails_model');
                      	$this->emails_model->offerContactEmail();
                      	$this->session->set_flashdata('flashmsgs', "Email Sent"); 
                      	redirect(base_url()."offers/".$this->data['offer']->slug); 
					  ...
					  $this->theme->view('offers/offer', $this->data);
				}
                else {
						$this->listing();
				}
		...
		function listing($offset = null){
                ...
                $this->data['offers'] = $alloffers['allOffers']['offers'];
				...
				$this->theme->view('offers/offers', $this->data);
		}
