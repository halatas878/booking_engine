 ...
 // add hotel images by type
  ...
  // update hotel image by type
   ...
   $data = array(
            ...
            'vid_approved' =>   $this->input->post('apprej')
  ...
  $this->db->delete('pt_room_images');
    ...
    $this->db->select('vid_link,vid_hotel_id');
  $this->db->where('vid_hotel_id',$hotelid);
  $hvids = $this->db->get('pt_hotel_videos')->result();


  foreach($hvids as $v){

    @unlink(PT_HOTELS_VIDEOS_UPLOAD.$v->vid_link);
  ...
  $this->db->delete('pt_hotels');
   ...
   $approval = pt_admin_gallery_approve();
   $data = array(
            'vid_hotel_id' => $hotelid,
            'vid_type' => $type,
            'vid_link' => $link,
            'vid_order' => $order,
            'vid_approved' => $approval
   );
...
$data['basic'] = $query[0]->hp_basic;
 ...
 $days = pt_count_days($checkin,$checkout);
  ...
  $this->db->select('pt_hotels.*,pt_cities.*,pt_states.*,pt_countries.*');
  $this->db->select_avg('pt_reviews.review_overall','overall');
