...
// Delete Multiple Socials
		...
		public function delete_multiple_socials() {
...
// Delete Multiple Countries
		...
		public function delete_multiple_countries() {
...
// Delete Single Country
		...
		public function delete_single_country() {
				$countriesid = $this->input->post('countriesid');
				$this->countries_model->delete_country($countriesid);
				$this->session->set_flashdata('flashmsgs', "Deleted Successfully");
		}
// Disable Multiple Countries

		public function disable_multiple_countries() {
				$countriesid = $this->input->post('countriesid');
				foreach ($countriesid as $id) {
						$this->countries_model->disable_country($id);
				}
				$this->session->set_flashdata('flashmsgs', "Disabled Successfully");
		}
// enable Multiple Countries

		public function enable_multiple_countries() {
				$countriesid = $this->input->post('countriesid');
				foreach ($countriesid as $id) {
						$this->countries_model->enable_country($id);
				}
				$this->session->set_flashdata('flashmsgs', "Enabled Successfully");
		}
// Delete Multiple States

		public function delete_multiple_states() {
				$statesid = $this->input->post('statesid');
				foreach ($statesid as $id) {
						$this->countries_model->delete_state($id);
				}
				$this->session->set_flashdata('flashmsgs', "Deleted Successfully");
		}
// Disable Multiple States

		public function disable_multiple_states() {
				$statesid = $this->input->post('statesid');
				foreach ($statesid as $id) {
						$this->countries_model->disable_state($id);
				}
				$this->session->set_flashdata('flashmsgs', "Disabled Successfully");
		}
// Disable Multiple States

		public function enable_multiple_states() {
				$statesid = $this->input->post('statesid');
				foreach ($statesid as $id) {
						$this->countries_model->enable_state($id);
				}
				$this->session->set_flashdata('flashmsgs', "Enabled Successfully");
		}
// Delete Single State

		public function delete_single_state() {
				$statesid = $this->input->post('statesid');
				$this->countries_model->delete_state($statesid);
				$this->session->set_flashdata('flashmsgs', "Deleted Successfully");
		}
// Delete Multiple Cities

		public function delete_multiple_cities() {
				$citiesid = $this->input->post('citiesid');
				foreach ($citiesid as $id) {
						$this->countries_model->delete_city($id);
				}
				$this->session->set_flashdata('flashmsgs', "Deleted Successfully");
		}
// Disable Multiple Cities

		public function disable_multiple_cities() {
				$citiesid = $this->input->post('citiesid');
				foreach ($citiesid as $id) {
						$this->countries_model->disable_city($id);
				}
				$this->session->set_flashdata('flashmsgs', "Disabled Successfully");
		}
// Enable Multiple Cities

		public function enable_multiple_cities() {
				$citiesid = $this->input->post('citiesid');
				foreach ($citiesid as $id) {
						$this->countries_model->enable_city($id);
				}
				$this->session->set_flashdata('flashmsgs', "Enabled Successfully");
		}
// Delete Single City

		public function delete_single_city() {
				$citiesid = $this->input->post('citiesid');
				$this->countries_model->delete_city($citiesid);
				$this->session->set_flashdata('flashmsgs', "Deleted Successfully");
		}
// delete multiple Slides

		public function delete_multiple_slides() {
...
// delete combination of city,state,country
		function del_cobmined() {
				$id = $this->input->post('id');
				$this->countries_model->delete_combined($id);
		}
...
// Delete multiple extras
		...
		public function delete_multiple_supps() {
				$supplist = $this->input->post('supplist');
				foreach ($supplist as $id) {
						$this->extras_model->delete_supp($id);
				}
				$this->session->set_flashdata('flashmsgs', "Deleted Successfully");
		}
...
<div class='alert alert-danger'>" . trans("0313") . "</div>";
		...
		function quicksms() {
				...
				elseif ($msg == "") {
						...
						$this->smsaddonlib->send_quick_sms($this->input->post());
						echo "1";
				}
		}
       ...
       function checkCoupon(){
		...
		function getCancellation($id=""){
			...
			$data = $this->modules_model->getCancellation($id);
			$hasil = array();
			foreach($data as $k=>$v){
		...
		function getRoom(){
			$id = $_GET['id'];
			$this->load->model('modules_model');
			$data = $this->modules_model->getRooms($id);
			$hotel = $this->modules_model->getHotel($id);
			$this->session->set_userdata(array("hotel"=>$id,"hotel_slug"=>$hotel[0]->hotel_slug));
			header('Content-Type: application/json');
			echo json_encode($data);
		}
		
		function newpostPrice($slug=""){
			$this->load->model('modules_model');
			$_POST['user_id'] = $this->session->userdata('pt_logged_id');
			$_POST['hotel_id'] = $this->session->userdata('hotel');
			$this->modules_model->updateNewRatetiers($_POST);			
			redirect($this->uri->segment(1).'/hotels/updaterates?room='.$_POST['room_id'].'&type=setup');
			exit();
		}
		
		function postPrice($slug=""){
			$this->load->model('modules_model');
			if(!isset($_GET['prices']))
				$data = $this->modules_model->updateRates($_POST);
			if(isset($_GET['action'])){
				$tipe = $_GET['action'];
				switch($tipe){
					case "update_rates":
						if(isset($_POST['priccesid'])){
							$data = $this->modules_model->editUpdateRates($_POST);
						}else{
							$data = $this->modules_model->newUpdateRates($_POST);
						}
						
						redirect($this->uri->segment(1).'/hotels/updaterates?room='.$_POST['room_id'].'&type=setup');
						break;
					default:
						redirect($this->uri->segment(1).'/hotels/updaterates?room='.$_POST['roomID'].'&type=setup');
						break;
				}
			}
			redirect($this->uri->segment(1).'/hotels/setuprates?room='.$_POST['roomID'].'&type=setup');
		}
