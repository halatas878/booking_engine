...
// $this->data['modModel'] = $this->modules_model;
				...
				if (!empty ($filter)) {
						...
						if ($this->data['type'] == 'videos') {
								if ($this->data['module'] == "hotels") {
										...
										$this->data['hotelvids'] = $this->get_all_pending_videos('hotels');
								}
								elseif ($this->data['module'] == "tours") {
										$chking = $this->ptmodules->is_mod_available_enabled("tours");
										if (!$chking) {
										...
										$this->data['tourvids'] = $this->get_all_pending_videos('tours');
								}
								elseif ($this->data['module'] == "cruises") {
										$chking = $this->ptmodules->is_mod_available_enabled("cruises");
										if (!$chking) {
										...
										$this->data['cruisevids'] = $this->get_all_pending_videos('cruises');
								}
						}
		...
		public function get_all_pending_videos($module) {
				if ($module == "hotels") {
						$this->db->where('vid_approved', '0');
						return $this->db->get('pt_hotel_videos')->result();
				}
				elseif ($module == "cruises") {
						$this->db->where('vid_approved', '0');
						return $this->db->get('pt_cruise_videos')->result();
				}
		...
		function accept_single_video() {
				$id = $this->input->post('id');
				$module = $this->input->post('moduletype');
				if ($module == "hotels") {
						$data = array('vid_approved' => '1');
				...
				elseif ($module == "cruises") {
						$data = array('vid_approved' => '1');
						$this->db->where('vid_id', $id);
						$this->db->update('pt_cruise_videos', $data);
				}
		}
		...
		function accept_multiple_videos() {
				$vidlist = $this->input->post('vidlist');
				$module = $this->input->post('moduletype');
		...
		function reject_single_video() {
				$id = $this->input->post('id');
				$module = $this->input->post('moduletype');
				$name = $this->input->post('vidname');
				$type = $this->input->post('vidtype');
				if ($module == "hotels") {
						$this->load->model("hotels/hotels_model");
						$this->hotels_model->delete_vids($name, $type, $id);
				}
				elseif ($module == "tours") {
						$this->load->model("tours/tours_model");
						$this->tours_model->delete_vids($name, $type, $id);
				}
				elseif ($module == "cruises") {
						$this->load->model("cruises/cruises_model");
						$this->cruises_model->delete_vids($name, $type, $id);
				}
		}

		function reject_multiple_videos() {
				$this->load->model("hotels/hotels_model");
				$vidlist = $this->input->post('vidlist');
				$module = $this->input->post('moduletype');
				foreach ($vidlist as $vid) {
						if ($module == "hotels") {
								$this->db->select('vid_type,vid_link');
								$this->db->where('vid_id', $vid);
								$rs = $this->db->get('pt_hotel_videos')->result();
								$vidtype = $rs[0]->vid_type;
								$vidlink = $rs[0]->vid_link;
								$this->hotels_model->delete_vids($vidlink, $vidtype, $vid);
						}elseif ($module == "cruises") {
								$this->load->model("cruises/cruises_model");
								$this->db->select('vid_type,vid_link');
								$this->db->where('vid_id', $vid);
								$rs = $this->db->get('pt_cruise_videos')->result();
								$vidtype = $rs[0]->vid_type;
								$vidlink = $rs[0]->vid_link;
								$this->cruises_model->delete_vids($vidlink, $vidtype, $vid);
						}
				}
