...
class Modules_model extends CI_Model {
		...
		function __construct() {
				parent :: __construct();
				$this->defaultPercentageMobile = 5;
				$this->defaultPercentageB2B = 10;
		}
...
//get details of item of a given module
	...
	function cekRateTiers($date){
		$this->db->select('ratetiers_id');
		$this->db->where('ratetiers_date',$date);
		return $this->db->get('ratetiers')->result();;
	}
	
	function rateInsert($args){
		return $this->db->insert("ratetiers", $args);
	}
	
	function rateUpdate($args,$id){
		$this->db->where("ratetiers_id",$id);
		return $this->db->update("ratetiers",$args);
	}
	
	function updateNewRatetiers($args){
		if(count($args['listrate'])==0) return false;
		$result = array();
		foreach($args['listrate'] as $k=>$v):
			$v 		= str_replace(",", "", $v);
			...
			$data	= array(
						...
						"rate_bar"		=>$v,
						"rate_mobile"	=>$v-(($this->defaultPercentageMobile/100)*$v),
						"rate_b2b"		=>$v-(($this->defaultPercentageB2B/100)*$v),
						"allotments"	=>$args['listalt'][$k],
						"in_active"		=>(isset($args['listclose'][$k]) && $args['listclose'][$k]=="1")?"0":"1",
						"role_id"		=>$args['cancellationPolicy']
					);
			$cek	= $this->cekRateTiers($date);
			(!$cek)?$this->rateInsert($data):$this->rateUpdate($data,$cek[0]->ratetiers_id);
			$result[] = $data;
