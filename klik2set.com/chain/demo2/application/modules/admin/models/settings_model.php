...
// update general settings
		function update_settings() {
				...
				$data = array('offline_message' => $this->input->post('offlinemsg'),'rss' => $this->input->post('rss'),'default_theme' => $this->input->post('theme'),'site_offline' => $this->input->post('site_offine'), 'site_title' => $this->input->post('site_title'), 
					...
					'default_lang' => $this->input->post('default_lang'), 'multi_lang' => $this->input->post('multi_lang'), 'default_city' => $this->input->post('default_city'), 'date_f' => $dateformat, 'date_f_js' => $jsdate, 'booking_expiry' => $expiry, 'booking_cancellation' => $cancel, 
					'coupon_code_length' => $codelength, 'coupon_code_type' => $this->input->post('coupon_code_type'), 'searchbox' => $this->input->post('searchbox'), 'allow_registration' => $this->input->post('allow_registration'),'user_reg_approval' => $this->input->post('user_reg_approval'), 
					'allow_supplier_registration' => $this->input->post('allow_supplier_registration'), 'multi_curr' => $this->input->post('multicurr'), 'updates_check' => $this->input->post('updates'), 'restrict_website' => $this->input->post('restrict'), 'mobile_pic_status' => $mobilePicStatus,'mobile_pic_url' => $mobilePicUrl,
					'mobile_redirect_status' => $mobileRedirectStatus,'mobile_redirect_url' => $mobileRedirectUrl);
				$this->db->where('user', 'webadmin');
				$this->db->update('pt_app_settings', $data);
...
// update security settings
		function update_security() {
				$data = array('secure_admin_key' => $this->input->post('secure_admin_key'), 'secure_admin_status' => $this->input->post('secure_admin_status'), 'secure_supplier_key' => $this->input->post('secure_supplier_key'), 'secure_supplier_status' => $this->input->post('secure_supplier_status'));
				$this->db->where('user', 'webadmin');
				$this->db->update('pt_app_settings', $data);
		}
...
// get watermark settings
		...
		function update_watermark_img($filename) {
				$data = array('wm_image' => $filename);
				$oldwm = $this->input->post('oldwm');
				if ($filename == $oldwm) {
				}
				else {
						if (!empty ($oldwm)) {
								@ unlink(PT_GLOBAL_UPLOADS_FOLDER . $oldwm);
						}
				}
				$this->db->where('wm_name', 'wmark');
...
// update watermark data
		function update_watermark_data() {
				...
				$data = array('wm_type' => $this->input->post('type'), 'wm_position' => $this->input->post('img_position'), 'wm_trans' => $this->input->post('img_trans'), 'wm_text' => $this->input->post('watermark_text'), 'wm_font_color' => $fontcolor, 'wm_font_size' => $this->input->post('font_size'), 'wm_status' => $this->input->post('wm_status'));
				$this->db->where('wm_name', 'wmark');
				$this->db->update('pt_watermark', $data);
		}
