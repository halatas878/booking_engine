  ...
  $.alert.open('confirm', 'Are you sure you want to Approve these Images', function(answer) {
     ...
     $.post("<?php echo base_url();?>admin/pending/accept_single_image", { id: id, moduletype: moduletype}, function(theResponse){
      ...
      if (answer == 'yes'){
       ...
       //Accept multiple Videos
        $('.accept_selected_videos').click(function(){
        var vidlist = new Array();
        $("input:checked").each(function() {
             vidlist.push($(this).val());
          });
        var count_checked = $("[name='videos_ids[]']:checked").length;
        if(count_checked == 0) {
  $.alert.open('info', 'Please select Videos to Approve.');
          return false;}

  $.alert.open('confirm', 'Are you sure you want to Approve these Videos', function(answer) {
      if (answer == 'yes')
     $.post("<?php echo base_url();?>admin/pending/accept_multiple_videos", { vidlist: vidlist, moduletype: moduletype}, function(theResponse){
     location.reload();
     });});});

      // accept single Video
       $(".accept_video_single").on('click',function(){
         var id = $(this).prop('id');

       $.alert.open('confirm', 'Are you sure you want to accept this Video', function(answer) {
      if (answer == 'yes'){

     $.post("<?php echo base_url();?>admin/pending/accept_single_video", { id: id, moduletype: moduletype}, function(theResponse){
                  $("#row_"+id).fadeOut("slow");
                  var pendings = $(".totalpendings").text();
                  var newpending = parseFloat(pendings) - 1;
                  if(moduletype == 'hotels'){
                  var hvpendings = $(".totalhotelvideos").text();
                  var newhvpending = parseFloat(hvpendings) - 1;
                   $(".totalhotelvideos").html(newhvpending);
                  }else if(moduletype == 'tours'){
                  var tvpendings = $(".totaltourvideos").text();
                  var newtvpending = parseFloat(tvpendings) - 1;
                   $(".totaltourvideos").html(newtvpending);
                  }
                $(".totalpendings").html(newpending);

     	});}});});
           //Reject multiple Videos
        $('.rej_selected_videos').click(function(){
        var vidlist = new Array();
        $("input:checked").each(function() {
             vidlist.push($(this).val());
          });
        var count_checked = $("[name='videos_ids[]']:checked").length;
        if(count_checked == 0) {
  $.alert.open('info', 'Please select Videos to Reject.');
          return false;}

  $.alert.open('confirm', 'Are you sure you want to Reject these Videos', function(answer) {
      if (answer == 'yes')
     $.post("<?php echo base_url();?>admin/pending/reject_multiple_videos", { vidlist: vidlist, moduletype: moduletype}, function(theResponse){
     location.reload();
     });});});

      // reject single Video
       $(".reject_video_single").on('click',function(){
         var id = $(this).prop('id');
         var vidname = $(this).data('videoname');
         var vidtype = $(this).data('videotype');

       $.alert.open('confirm', 'Are you sure you want to reject this Video', function(answer) {
      if (answer == 'yes'){
       $.post("<?php echo base_url();?>admin/pending/reject_single_video", { id: id, vidtype: vidtype, vidname: vidname, moduletype: moduletype}, function(theResponse){
                  $("#row_"+id).fadeOut("slow");
                  var pendings = $(".totalpendings").text();
                  var newpending = parseFloat(pendings) - 1;
                 if(moduletype == 'hotels'){
...
<div class="<?php echo body;?>">
  ...
  <div class="panel panel-primary table-bg">
   ...
   <option value="customers" <?php if($module == "customers"){ echo "selected"; } ?> >Customers</option>
    ...
    Images: <?php echo $pendingitems['img_hotels']; ?>&nbsp;&nbsp;Room Images: <?php echo $pendingitems['img_rooms']; ?>&nbsp;&nbsp; Videos: <?php echo $pendingitems['vid_hotels']; ?>&nbsp;&nbsp; Reviews: <?php echo $pendingitems['rev_hotels']; ?>
    <hr><?php } ?>
    <?php if($chkingtours){ ?><h3>Tours</h3>
    Images: <?php echo $pendingitems['img_tours']; ?>&nbsp;&nbsp; Videos: <?php echo $pendingitems['vid_tours']; ?>&nbsp;&nbsp; Reviews: <?php echo $pendingitems['rev_tours']; ?>
    <hr> <?php } ?>
    ...
    <!----Vidoes pending Starting---->
    <!-- PHPTRAVELS Videos table starting -->
    <table cellpadding="0" cellspacing="0" border="0" class="datatable table table-hover table-striped table-bordered">
      <thead>
        <tr>
          <th><input class="pointer" type="checkbox" data-toggle="tooltip" data-placement="top" title="Select All" id="select_all"  /></th>
          <th><span class="fa fa-user" data-toggle="tooltip" data-placement="top" title="Video Name"></span> Video </th>
          <th><i class="fa fa-wrench" data-toggle="tooltip" data-placement="top" title="Action"></i> Action</th>
        </tr>
      </thead>
      <tbody>
        <?php
          if($module == "hotels"){
          foreach($hotelvids as $vids){
             ?>
        <tr id="row_<?php echo $vids->vid_id;?>">
          <td><input type="checkbox" name="videos_ids[]" value="<?php echo $vids->vid_id;?>" class="selectedId"  /></td>
          <td>
            <div class="col-lg-3" id="d_<?php echo $vids->vid_id;?>" name="videos">
              <div class="thumbnail wobble-vertical">
                <?php
                  if($vids->vid_type != 'uploads'){

                  echo pt_embed_videos($vids->vid_link);
            ...
            <!-- <a class="colorbox" href="<?php // echo pt_image_paths_constants($img->himg_type,'hotels').$img->himg_image;?>"><?php echo $vids->vid_link;?> </a>-->
          </td>
          <td align="center">
            <span class="btn btn-xs btn-success accept_video_single" id="<?php echo $vids->vid_id;?>"><i class="fa fa-check"></i> Approve</span>
            <span class="btn btn-xs btn-danger reject_video_single" id="<?php echo $vids->vid_id;?>" data-videoname="<?php echo $vids->vid_link;?>" data-videotype="<?php echo $vids->vid_type;?>" ><i class="fa fa-times"></i> Delete</span>
          </td>
        </tr>
        <?php } }elseif($module == "tours"){
          foreach($tourvids as $vids){
             ?>
        <tr id="row_<?php echo $vids->vid_id;?>">
          <td><input type="checkbox" name="videos_ids[]" value="<?php echo $vids->vid_id;?>" class="selectedId"  /></td>
          <td>
            <div class="col-lg-3" id="d_<?php echo $vids->vid_id;?>" name="videos">
              <div class="thumbnail wobble-vertical">
                <?php
                  ...
                  ?>
                <video width="200" height="150" controls>
                  <source src="<?php echo PT_TOURS_VIDEOS.$vids->vid_link;?>">
                  Your browser does not support the video tag.
                </video>
                <?php
                  }
