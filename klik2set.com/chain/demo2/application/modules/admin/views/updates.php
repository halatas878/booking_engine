<div class="panel panel-default">
 ...
 <?php } ?>
  <?php if(empty($updatesList)){ echo "<h2>Please check back a moment later"; }else{ 
            if($allUpdated){ echo "No more Updates available"; }else{ ?>
  ...
  <table cellpadding="0" cellspacing="0" border="0" class="table table-hover table-striped table-bordered">
      ...
      <tbody>
       <?php $chkUpdate = array_diff($allUpdates,$updatesDone->updated); $checkingInstall = min($chkUpdate); foreach($updatesList as $update){ if(!in_array($update->update,$updatesDone->updated) && $ptVersion == trim($update->ptversion)){ ?> 
