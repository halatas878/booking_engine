...
class Carsback extends MX_Controller {
		...
		function index() {
			if(!$this->data['addpermission'] && !$this->editpermission && !$this->deletepermission){
                ...
                $delurl = base_url().'cars/carajaxcalls/delCar';	
                $xcrud->button("javascript: delfunc('{car_id}','$delurl')",'DELETE','fa fa-times', 'btn-danger',array('target'=>'_self','id' => '{car_id}'));
                }
                $xcrud->limit(50);
				$xcrud->unset_add();
				$xcrud->unset_edit();
				...
				$xcrud->columns('car_is_featured,thumbnail_image,car_title,car_stars,Owned By,car_location,car_slug,car_order,car_status');
		...
		function add() {
				...
				if (!empty ($addcar)) {
						$this->form_validation->set_rules('carname', 'Car Name', 'trim|required|xss_clean');
...
*/
						...
						if ($this->form_validation->run() == FALSE) {
								echo '<div class="alert alert-danger">' . validation_errors() . '</div><br>';
										...
										echo "done";
										$this->session->set_flascdata('flashmsgs', 'Car added Successfully');
								}
				}
				else {
...
// Upload car Videos
		...
		public function do_video_upload() {
				$this->load->model('cars/cars_uploads_model');
				echo $this->cars_uploads_model->__car_videos();
		}
// Delete car videos

		public function deletevids($file) {
				@ unlink(PT_CARS_VIDEOS_UPLOAD . $file);
				$this->db->where('vid_link', $file);
