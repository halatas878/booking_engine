...
if (!defined('BASEPATH'))
...
} if (!function_exists('getCancelations')) {
	function getCancelations($id=0){
		if($id>0){
			$CI = get_instance();
	        $CI->load->model('hotels/hotels_model');
	        $res = $CI->hotels_model->getCancelations($id);
	        return $res;
		}else{
	      return '';
		}
	...
	function getShortRoom($id,$hotel){
		
		if($id>0){
			$CI = get_instance();
	        $CI->load->model('hotels/hotels_model');
	        $res = $CI->hotels_model->getShortRoom($id,$hotel);
	        return $res;
		}else{
	      return '';
		}
...
} if (!function_exists('getHotelRooms')) { 
	function getHotelRooms($id=0){
		
		if($id>0){
			$CI = get_instance();
	        $CI->load->model('hotels/hotels_model');
	        $res = $CI->hotels_model->getHotelRooms($id);
	        return $res;
		}else{
	      return '';
		}
