...
// get hotel rooms
		function hotel_rooms($hotelid = null,$checkin = null,$checkout = null) {
				...
				if(!empty($_GET['Promocode'])){
					...
					$cekCoupons = $this->cekCouponsvalid($_GET['Promocode'],$hotelid,$checkin,$checkout);
					#echo "<pre/>";print_r($cekCoupons);exit('asas');
					if(!$cekCoupons){
						return array();
					}else{
						$datacoupons = $cekCoupons;
					}
				}
		...
		function cekCouponsvalid($code,$hotelid,$checkin,$checkout){
			$this->db->select('*');
			
			if(!empty($checkin)) $checkin = (date("Y-m-d",strtotime(str_replace("/","-", $checkin))));
			#return ($checkin);
			$this->db->where("`status` = 'yes' and DATE_FORMAT(FROM_UNIXTIME(startdate),'%Y-%m-%d')<=date_format('$checkin','%Y-%m-%d') and DATE_FORMAT( FROM_UNIXTIME(expirationdate),'%Y-%m-%d')>=date_format('$checkin','%Y-%m-%d')  and (item='all' or item='$hotelid') and code='$code'");
...
// get hotel rooms with limited details
 ...
 // Room Price
		...
		function hotel_details() {
				...
				$slug = $details[0]->hotel_slug.$this->checkinout;
			   ...
			   //	$this->isfeatured = $this->is_featured();
                ...
                $detailResults = (object)array('id' => $details[0]->hotel_id, 'title' => $title, 'slug' => $slug,'thumbnail' => $thumbnail,
            'stars' => pt_create_stars($stars), 'starsCount' => $stars, 'location' => $city->city,'desc' => $desc,'amenities' => $amenities,
            'latitude' => $latitude,'longitude' => $longitude,'sliderImages' => $sliderImages,'relatedHotels' => $relatedHotels,'paymentOptions' => $paymentOptions,
            'defcheckin' => $defcheckin, 'defcheckout' => $defcheckout,'metadesc' => $metadesc,'keywords' => $keywords,'policy' => $policy,'tripadvisorid' => $tripadvisorid);

                return $detailResults;
		}

		function hotel_short_details() {
	   $this->db->select('hotel_id,hotel_phone,hotel_email,hotel_website,hotel_title,hotel_desc,hotel_policy,tripadvisor_id,hotel_city,hotel_basic_price,hotel_basic_discount,hotel_is_featured,

   hotel_trusted,hotel_best_price,hotel_stars,hotel_slug,hotel_refundable,hotel_ratings,hotel_arrivalpay,thumbnail_image,hotel_amenities,hotel_latitude,hotel_longitude,hotel_meta_keywords,hotel_meta_desc');

                $this->db->where('hotel_id', $this->hotelid);
				$details = $this->db->get('pt_hotels')->result();
				$this->tripadvisorid = $details[0]->tripadvisor_id;
				$this->title = $this->get_title($details[0]->hotel_title);
				$this->slug = $details[0]->hotel_slug.$this->checkinout;
...
//  $this->db->or_where('extras_forever','forever');
       ...
       function getHotelTypes(){
         ...
         foreach($htypes as $htype){
             ...
             $result[] = array("id" => $htype->sett_id,"name" => $trans['name']);
         }
        return $result;
       }
        ...
        function getHotelAmenities(){
         ...
         foreach($amts as $amt){
             ...
             $result[] = array("id" => $amt->sett_id,'icon' => PT_HOTELS_ICONS.$amt->sett_img,"name" => $trans['name']);
         }
        return $result;
       }
...
// hotel Reviews
		function hotelReviews($hotelid = null,$offset=0) {
			if(empty($hotelid)){
				$hotelid = $this->hotelid;
			}
...
// hotel Reviews Averages
		function hotelReviewsAvg($hotelid) {
                if(empty($hotelid)){
                  $hotelid = $this->hotelid;
                }
		...
		function getLocationsList(){
			...
			$this->db->select('hotel_title,hotel_city');
			...
			foreach($locations as $loc){
				...
				$resultLocations[] = (object)array('id' => $locInfo->id,'name' => $locInfo->city,'hotel_name'=>$loc->hotel_title);
		...
		function has_videos() {
				$this->db->where('vid_hotel_id', $this->hotelid);
				$this->db->where('vid_approved', '1');
				$this->db->order_by('vid_order', 'asc');
				$q = $this->db->get('pt_hotel_videos')->num_rows();
				if ($q > 0) {
						return true;
				}
				else {
		...
		function room_short_details($id) {
			   	$this->db->select('room_id,room_quantity,room_basic_price,room_title,room_desc,room_adults,room_children,room_amenities,thumbnail_image,extra_bed,extra_bed_charges');
		...
		function featured_hotels_list() {
				...
				$this->db->select('hotel_id,hotel_order,hotel_title,hotel_status');
				$this->db->where('hotel_is_featured', 'yes');
			 	$this->db->where('hotel_featured_from <', time());
			 	$this->db->where('hotel_featured_to >', time());
        ...
        function getFeaturedHotels($ismobile){
          $hotels = $this->featured_hotels_list();
          $result = $this->getResultObject($hotels,$ismobile);
          return $result;
        }
...
// List 2 top rated hotels from each city passed to to this function
        ...
        function bestPrice($hotelid = null,$ismobile=0){
          if(empty($hotelid)){
            $hotelid = $this->hotelid;
          }
          $this->ci->load->model('hotels/rooms_model');
            ...
            $res = $this->db->get('pt_rooms')->result();
            foreach($res as $r){
             $p = $this->ci->rooms_model->getRoomPrice($r->room_id,$this->checkin,$this->checkout,$ismobile);
             $result[] = $p['perNight'];
           //  $result[] = $p;
            }
        ...
        function setTax($amount){
         ...
         function getResultObject($hotels,$ismobile=0){
          $this->ci->load->library('currconverter');
          $result = array();
          $curr = $this->ci->currconverter;
          ...
          foreach($hotels as $h){
            ...
            $bestprice = $this->bestPrice($ismobile);
            $price = $curr->convertPrice($bestprice,0);
         ...
         if(!empty($_GET['price'])){
			...
			$result[] = (object)array('id' => $this->hotelid, 'title' => $this->title, 'slug' => base_url().'hotels/'.$this->slug,'thumbnail' => $this->thumbnail,
			'stars' => pt_create_stars($this->stars),'starsCount' => $this->stars, 'location' => $this->location,'desc' => strip_tags($this->desc),
			'price' => $price,'currCode' => $curr->code,'currSymbol' => $curr->symbol,'amenities' => $this->amenities,
        ...
        //make a result object of Rooms Array
         function getRoomsResultObject($rooms,$checkin = null, $checkout = null){
          ...
          foreach($rooms as $room){
            ...
            'maxAdults' => $details[0]->room_adults,'masterRate' => $details[0]->room_basic_price,'maxChild' => $details[0]->room_children,
            'maxQuantity' => $availQuantity, 'thumbnail' => PT_ROOMS_THUMBS.$details[0]->thumbnail_image, 'Images' => $images,
        ...
        //make a result object of Rooms Array
         function getBookResultObject($hotelid,$roomid,$roomscount,$extrabeds = 0,$checkin = null,$checkout = null){
			...
			if (isset($_GET['promotions']) && isset($roomprice['promotions'][$_GET['promotions']])){
				$promotionsID = ($_GET['promotions']);
				$perNight = $roomprice['promotions'][$promotionsID]['curency'];
				$roomprice['perNight'] = $perNight;
				$toPrice = $roomprice['totalPrice'] = $perNight*$roomprice['stay'];
			}else{
				$perNight =$roomprice['perNight'];
				$toPrice = $roomprice['totalPrice'];
			}
            ...
            $totalSum = ($toPrice * $roomscount) + $extrabedcharges;
          ...
          $result["room"] = (object)array('id' => $roomid, 'basic_price' => $details[0]->room_basic_price,'title' => $this->roomtitle, 'desc' => $this->roomdesc,
        ...
        //end room details for booking page
            ...
            $result["hotel"] = (object)array('id' => $this->hotelid, 'title' => $this->title, 'slug' => base_url().'hotels/'.$this->slug,'thumbnail' => $this->thumbnail,
            'stars' => pt_create_stars($this->stars),'starsCount' => $this->stars, 'location' => $this->location,'checkin' => $checkin, 'checkout' => $checkout,
            'metadesc' => $this->metadesc,'keywords' => $this->keywords,'extras' => $extras,'taxAmount' => $taxAmount,'depositAmount' => $depositAmount,
            'policy' => $this->policy,'extraChkUrl' => $extrasCheckUrl,'adults' => $this->adults,'children' => $this->children);
        ...
        //get updated values of booking data after extras and payment method updates
         function getUpdatedDataBookResultObject($hotelid,$roomid,$checkin,$checkout,$roomscount,$extras,$extrabeds = 0){
          ...
          $taxAmount = $curr->convertPrice($this->taxamount,0);
          $grandTotal = $total + $paymethodTotal + $this->taxamount;	
          $this->setDeposit($grandTotal);

          $depositAmount = $curr->convertPrice($this->deposit,0);
          $bedcharges = $curr->convertPrice($extrabedcharges,0);

          
          $price = $curr->convertPrice($grandTotal,0);
          $perNight = $curr->convertPriceFloat($roomprice['perNight'],0);
          $extrasHtml = "";
		...
		public function intervalDate($begin,$end){
			$end = ($this->convertDate($end));
			$begin = ($this->convertDate($begin));
