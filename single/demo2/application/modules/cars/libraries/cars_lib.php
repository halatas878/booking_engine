...
//get the CI instance
                ...
                $searchVars = "?&pickupLocation=".$this->pickupLocation."&dropoffLocation=".$this->dropoffLocation."&pickupDate=".$this->pickupDate."&pickupTime=".$this->pickupTime;
...
//set car id by id
		...
		function listgrid() {
				...
				return $settings[0]->front_listings_view;
		}
...
// get car images
        ...
        $curr = $this->ci->currconverter;
				...
				$slug = $details[0]->car_slug.$this->urlVars;
                ...
                $latitude = $details[0]->car_latitude;
                $longitude = $details[0]->car_longitude;

                $carPrice = $this->carAmount($carid,$this->pickupLocation, $this->dropoffLocation);
                ...
                $detailResults = (object)array('id' => $details[0]->car_id, 'title' => $title, 'slug' => $slug,'thumbnail' => $thumbnail,
           		 'stars' => pt_create_stars($stars), 'starsCount' => $stars, 'location' => $location,'desc' => $desc,'latitude' => $latitude,'longitude' => $longitude,'sliderImages' => $sliderImages,'relatedCars' => $relatedCars,'paymentOptions' => $paymentOptions,
          		 'metadesc' => $metadesc,'keywords' => $keywords,'policy' => $policy,'currCode' => $curr->code,'currSymbol' => $curr->symbol,'date' => $this->date,'totalCost' => $curr->convertPrice($totalCost,0),'comType' => $comm_type, 'comValue' => $comm_value, 'taxType' => $tax_type, 'taxValue' => $tax_value,
                 'carPrice' => $carPrice,'totalDeposit' => $depositAmount, 'taxAmount' => $this->taxamount, 'showTotal' => $showTotal);
		...
		function car_short_details($carid = null) {
			...
			if(empty($carid)){
				...
				$this->slug = $details[0]->car_slug.$this->urlVars;
				...
				$this->latitude = $details[0]->car_latitude;
				$this->longitude = $details[0]->car_longitude;
...
// car Price
	  ...
	  function getRelatedCars($cars){
       ...
       if(!empty($cars)){
		...
		function getDropoffLocations($location, $carid){
         ...
         function getLimitedResultObject($cars){
          ...
          if(!empty($cars)){
			...
			$price = $curr->convertPrice($this->basicprice,0);
         ...
         function getBookResultObject($carid,$date = null){
			if(empty($date)){
				$date = $this->date;
			}
            ...
            $subTotal = $curr->convertPrice($totalSum,0);

			$this->setTax($totalSum);
			$taxAmount = $curr->convertPrice($this->taxamount,0);
			$totalPrice = $curr->convertPriceFloat($totalSum, 0) + $curr->convertPriceFloat($this->taxamount, 0);
            ...
            $result["car"] = (object)array('id' => $this->carid, 'title' => $this->title, 'slug' => base_url().'cars/'.$this->slug,'thumbnail' => $this->thumbnail,
            ...
            'subTotal' => $subTotal,'pickupLocation' => $this->pickupLocation,'dropoffLocation' => $this->dropoffLocation, 'pickupTime' => $this->pickupTime, 'pickupDate' => $this->pickupDate);
...
//  $this->db->or_where('extras_forever','forever');
        ...
        //get updated values of booking data after extras and payment method updates
         function getUpdatedDataBookResultObject($carid, $extras, $pickup,$dropoff){
          $this->ci->load->library('currconverter');
          $result = array();
          $curr = $this->ci->currconverter;
          ...
          $totalSum = $this->carAmount($carid, $pickup, $dropoff);
          $total = $curr->convertPriceFloat($extTotal,0) + $curr->convertPriceFloat($totalSum,0);
         ...
         $subitem = "";//array("adults" => $adultsSubitem, "child" => $childSubitem,"infant" => $infantSubitem);
          ...
          $result = (object)array('grandTotal' => $price,'taxAmount' => $taxAmount, 
            	...
            	'bookingType' => "cars",'currCode' => $curr->code, 'stay' => 1,
        ...
        function carAmount($carid, $pickup = null, $dropoff = null){
        ...
        public function bookedInvoiceInfo($bookid){
        	...
        	if(!empty($res)){
        		...
        		$result->pickupDate = pt_show_date_php($res[0]->booked_pickupdate);
