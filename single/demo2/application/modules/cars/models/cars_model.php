...
// add car data
		function add_car($user = null) {
			if(empty($user)){
				...
				'car_stars' => $this->input->post('carstars'), 
				...
				'car_doors' => $this->input->post('doors'), 
				...
				'car_passengers' => $this->input->post('passangers'), 
...
// Get car settings data
		function get_car_settings_data($type) {
				if(!empty($type)){
...
// update video order
		function update_video_order($vid, $order) {
				...
				$this->db->update('pt_car_videos', $data);
		}
...
// Add video Link
		function add_video_link($link, $carid, $type) {
				$this->db->select('vid_car_id');
				$this->db->where('vid_car_id', $carid);
				$nums = $this->db->get('pt_car_videos')->num_rows();
				$order = $nums + 1;
				$approval = pt_admin_gallery_approve();
				$data = array('vid_car_id' => $carid, 'vid_type' => $type, 'vid_link' => $link, 'vid_order' => $order, 'vid_approved' => $approval);
				$this->db->insert('pt_car_videos', $data);
		}

// Get car videos
		function get_car_videos($id) {
				$this->db->where('vid_car_id', $id);
				$this->db->order_by('vid_order', 'asc');
				$q = $this->db->get('pt_car_videos');
...
// Delete car Video
		function delete_vids($name = null, $type = null, $id = null) {
				...
				if ($type == "uploads") {
						@ unlink(PT_CARSVIDEOS_UPLOAD . $name);
				}
				$this->db->where('vid_id', $id);
				$this->db->delete('pt_car_videos');
		}
...
// Search Cars By text
		function search_cars_by_text($perpage = null, $offset = null, $orderby = null, $cities = null) {
				...
				$this->db->select('pt_cars.*,pt_cities.*,pt_states.*,pt_countries.*');
//  $this->db->select_avg('pt_reviews.review_overall','overall');
				$this->db->join('pt_cities', 'pt_cars.car_city = pt_cities.city_id', 'left');
...
//  $this->db->join('pt_reviews','pt_cars.car_id = pt_reviews.review_itemid','left');
				...
				$this->db->or_like('pt_countries.short_name', $txtsearch);
				$this->db->or_like('pt_states.state_name', $txtsearch);
				$this->db->or_like('pt_cities.city_name', $txtsearch);
				if ($orderby == "za") {
						$this->db->order_by('pt_cars.car_title', 'desc');
				}
				elseif ($orderby == "az") {
...
// for auto suggestions search
	...
	UNION
          SELECT car_title as name FROM pt_cars WHERE car_title LIKE '%$term%' ")->result();
