...
class Rooms_model extends CI_Model{
  ...
  $quantity = $this->input->post('quantity');
   ...
   if($this->input->post('extrabeds') > 0){
   ...
   $data = array(
            'room_title' => $this->input->post('roomname'),
            'room_desc' => $this->input->post('roomdesc'),
            'room_hotel' => $this->input->post('hotelid'),
            'room_basic_price' => $this->input->post('basicprice'),
            'room_amenities' => $amenities,
            'room_type' => $this->input->post('roomtype'),
            'room_status' => $this->input->post('roomstatus'),
            'room_adults' => $this->input->post('adults'),
            'room_children' => $this->input->post('children'),
            'room_min_stay' =>  $this->input->post('minstay'),
            'extra_bed_charges' =>  $bedcharges,
            'extra_bed' =>  $this->input->post('extrabeds'),
  ...
  if($quantity < 1){
   ...
   $data = array(
            ...
            'room_children' => $this->input->post('children'),
            'room_min_stay' =>  $this->input->post('minstay'),
            'extra_bed_charges' =>  $this->input->post('bedcharges'),
            'extra_bed' =>  $this->input->post('extrabeds'),
            'room_quantity' => $quantity,
   );
    ...
    function updateRoomThumb($roomid,$imgname,$action) {
      if($action == "delete"){
        ...
        function addRoomPrices($roomid){
          ...
          'extra_bed_charge' => $this->input->post('bedcharges'),
        ...
        function updateRoomPrices($prices){
          ...
          'adults' => $v['adults'],
          'children' => $v['child'],
          'extra_bed_charge' => $v['extra_bed_charges'],
          'mon' => $v['mon'],
          'tue' => $v['tue'],
          'wed' => $v['wed'],
          'thu' => $v['thu'],
        ...
        function getRoomPrices($roomid,$hotelid=0){
          $this->db->where('room_id',$roomid);
          $this->db->order_by('date_from','ASC');
          return $this->db->get('pt_rooms_prices')->result();
        }
		
		function getPricesList($id){
          $this->db->where('id',$id);
          return $this->db->get('pricelist')->result();
        }

		function getHotelonRooms($roomid=0,$hotelid=0){
			if($hotelid>0)
				$this->db->where('room_hotel',$hotelid);	
			if($roomid>0)
          		$this->db->where('room_id',$roomid);
			$this->db->where('date_from not like "%0000-00-00%"');
          $this->db->order_by('date_from','ASC');
          return $this->db->get('pricelist')->result();
		}
      ...
      $this->db->query($sql);
		...
		function checkPromotions($roomid,$checkin,$checkout){
			$checkin = date("Y-m-d", strtotime($checkin));
			$checkout = date("Y-m-d", strtotime($checkout));
			$querys =$this->db->query("select promotions.*, pt_cancellation_policy.title, pt_cancellation_policy.description from promotions join pt_cancellation_policy on promotions.cancellation=pt_cancellation_policy.id where promotions.room_id='$roomid' and (DATE_FORMAT(promotions.start_stay_period,'%Y-%m-%d') <= DATE_FORMAT('$checkin','%Y-%m-%d') <= DATE_FORMAT(promotions.finish_stay_period,'%Y-%m-%d')) and (DATE_FORMAT(promotions.start_stay_period,'%Y-%m-%d') <= DATE_FORMAT('$checkout','%Y-%m-%d') <= DATE_FORMAT(promotions.finish_stay_period,'%Y-%m-%d'))");
			$hasil 	= array();
			$urut	= array();
			$promotions = $querys->result_array();
				if(empty($promotions)) return false;
			foreach($promotions as $k=>$v){
				$type				= $v['type'];
				$nights			 	= $this->getRateDay($checkin,$checkout);
				$urut[$k]			= $v['curency'];
				$start_book_period	= date("Y-m-d",strtotime($v['start_booking_period']));
				$finish_book_period	= date("Y-m-d",strtotime($v['finish_booking_period']));
				$now				= date("Y-m-d"); 
				if($start_book_period <= $now && $now <= $finish_book_period){
					if($type=="bonus-night" && ($nights!=$v['stay'])) continue;
					$hasil[$v['id']] = $v; 
				} 
			}
			array_multisort($urut, SORT_ASC,  $hasil);
			return $hasil;
		}

      function getRoomPrice($roomid,$checkin,$checkout,$ismobile=0){
		$promote = $this->checkPromotions($roomid, $checkin, $checkout);
		#echo "<pre/>"; print_r($promote);exit();
